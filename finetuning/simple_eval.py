...
def simple_evaluate_SAM(model, dataset, device, img_size, patch_size, thresholds=[0.05, 0.1, 0.2]):
    """
    Evaluate model on test set using PCK metric
    ...
    with torch.no_grad():
        for idx, sample in enumerate(dataset):
            ...
            # Extract features
            src_features = extract_dense_features_SAM(model, src_tensor, image_size=img_size, training=False)
            tgt_features = extract_dense_features_SAM(model, tgt_tensor, image_size=img_size, training=False)
